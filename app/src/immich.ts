...
class Immich {
  ...
  async request(endpoint: string) {
    try {
      ...
      log(
        `From the server IPP is running on, see if you can curl to ${this.apiUrl()}/server/ping and receive a JSON result.`,
      );
    }
  }
  ...
  async handleShareRequest(request: IncomingShareRequest, res: Response) {
    ...
    const sharedLinkRes = await immich.getShareByKey(
      request.key,
      request.password,
      request.keyType || KeyType.key,
    );
    ...
    if (!sharedLinkRes.link) {
      respondToInvalidRequest(
        res,
        404,
        "Unknown error with key " + request.key,
      );
      return;
    }
    ...
    if (
      request.password &&
      request.req.session &&
      !request.req.session[link.key]
    ) {
      request.req.session[link.key] = encrypt(
        JSON.stringify({
    ...
    if (request.mode === "download" && canDownload(link)) {
      ...
      if (
        ...
        // Show a gallery page
        const openItem = getConfigOption("ipp.singleItemAutoOpen", true)
          ? 1
          : 0;
        ...
        if (res.status === 200) {
          ...
          if (link.type === AlbumType.album) {
            const albumRes = await fetch(
              this.buildUrl(this.apiUrl() + "/albums/" + link?.album?.id, {
                [keyType]: key,
                password,
              }),
            );
          ...
          if (link.expiresAt && dayjs(link.expiresAt) < dayjs()) {
            ...
            if (!link.showMetadata) {
              // If the album doesn't have "show metadata" enabled, hide the EXIF data from the API response
              link.assets.forEach((asset) => {
            ...
            if (sortOrder === "asc") {
              link.assets.sort(
                (a, b) =>
                  a?.fileCreatedAt?.localeCompare(b.fileCreatedAt || "") || 0,
              );
            } else if (sortOrder === "desc") {
              link.assets.sort(
                (a, b) =>
                  b?.fileCreatedAt?.localeCompare(a.fileCreatedAt || "") || 0,
              );
            }
            return {
              valid: true,
  ...
  async getVideoContentType(asset: Asset) {
    const data = await this.request(
      this.buildUrl(
        "/assets/" + encodeURIComponent(asset.id) + "/video/playback",
        {
  ...
  formatMetadata(exifInfo: Asset["exifInfo"]) {
    // Remove all metadata not in the typescript definition
    if (!exifInfo) return null;
    for (const key in exifInfo) {
      if (!allowedExifKeys.includes(key as keyof typeof exifInfo)) {
        delete exifInfo[key as keyof typeof exifInfo];
      }
    }
    return exifInfo;
  }
