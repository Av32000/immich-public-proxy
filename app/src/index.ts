...
app.use(
  "/share/static",
  express.static("public", { setHeaders: addResponseHeaders }),
);
...
app.get(
  ...
  async (req, res) => {
    ...
    if (
      keyType === KeyType.slug &&
      !getConfigOption("ipp.allowSlugLinks", true)
    ) {
      // Slug type links are not allowed
      respondToInvalidRequest(
        res,
        404,
        "Slug links are disabled in config.json",
      );
    } else {
...
app.get(
  ...
  async (req, res) => {
    ...
    if (
      req.params.size &&
      !Object.values(ImageSize).includes(req.params.size as ImageSize)
    ) {
    ...
    const assetBelongsToShare = share.link?.assets?.some(
